...
path= './nerf/INRFWI_500k_polar_noise_6000epoch/'
...
def patch_cat(image_patch, n_row, n_col):
    ...
    for i in range(n_row):
        ...
        result = torch.cat(temp,dim = 2)  # 将拆分的图像块拼接成一行，沿着第2维度
        if i==0:
            output = result
        else:
            output = torch.cat([output,result],dim = 1)  # 沿着第1个维度进行拼接，最终形成完整图像
    return output
...
# # load signal
# source = scio.loadmat('./Phantom_20240123/source.mat')
# source_signal=np.array(source['source'])
# plt.plot(source_signal)
# source_signal = torch.tensor(1e7*source_signal.T,device = device, dtype = torch.float32)
...
v_true = torch.from_file('./breast/phantom_592_short_Neg35_with_tumour.bin',
...
# 添加高斯噪声
target_snr = 40 # 目标信噪比 (SNR, dB)
signal_power = observed_data.pow(2).mean()

noise_power = signal_power / (10 ** ((target_snr) / 10)) # 根据目标SNR计算噪声功率（高斯方差）
noise = torch.sqrt(noise_power) * torch.randn_like(observed_data) # 生成高斯噪声

noisy_observed_data = observed_data + noise

# 打印噪声加入后的 SNR 以验证
calculated_noise_power = ((observed_data - noisy_observed_data) ** 2).mean()
calculated_snr_db = 10 * torch.log10(signal_power / calculated_noise_power)
print(f'Calculated SNR (dB): {calculated_snr_db.item()}')
...
def FFP(x, y, b, input_size):
    ...
    # # 拼接原始坐标和编码后的坐标
    # coords = torch.stack([x, y], dim=-1)  # 原始坐标 (N, 2)
    # input_layer = torch.cat([coords, out], dim=-1)  # 拼接 (N, 2 + input_size)
    return out

def cart_to_polar(x, y, nx, ny):
    x = x.reshape(-1)
    y = y.reshape(-1)
...
input_size = 1000   # 映射后特征的维度
std = 0.1 
freq_num = int(input_size/2)  
# 傅里叶特征映射，通常将二维坐标(x,y)映射为一组高维的特征。映射通过将输入坐标应用到一组不同频率的正弦和余弦函数上实现。 # 对每个频率'i'计算一个正弦值和一个余弦值，会得到一对特征（即两个特征）
b = torch.normal(0,std,size=(2,freq_num))  # 符合正态分布的二维随机矩阵，均值为0，标准差为std。'b[0,i]'和'b[1,i]'分别表示第'i'个频率的两个不同分量，分别乘以'x'和'y'坐标，用于计算正弦和余弦值。 # 傅里叶特征映射中乘上随机矩阵'b'可能的目的：引入随机性以提升表达能力；增加特征的多样性；提升模型的泛化能力；提升对不同频率特征的感知
...
y_range = torch.linspace(2,(n_cols-1)*patch_size+2,n_cols)
x,y= torch.meshgrid(x_range, y_range, indexing='ij')

# 输入编码坐标
r, theta = cart_to_polar(x, y, nx, ny)
input_layer = FFP(r, theta, b, input_size).to(device)
# input_layer =  torch.stack([r, theta], dim=-1).reshape(-1, 2).to(device)  # (n_rows * n_cols, 2)
...
def adjust_learning_rate_warm_up(optimiser, epoch, start_lr, warmup_epochs=50):
    """
    Adjusts the learning rate with warm-up and exponential decay.
    During warm-up, the learning rate linearly increases to start_lr.
    After warm-up, it decays by 0.8 every 500 epochs.
    """
    if epoch < warmup_epochs:
        ...
        lr = start_lr * (0.8 ** ((epoch - warmup_epochs) // 500))
...
for epoch in range(n_epochs):  # 训练执行'n_epochs'此
    def closure():  # 每次训练都会调用内部的'closure'函数，该函数封装了整个前向传播、损失计算、反向传播和参数更新的过程
        ...
        if (((epoch_loss - loss_last)/loss_last > 0.3) or (math.isnan(epoch_loss))): 
            ...
            with open(path + 'pretrain_loss_list_train.pkl', 'wb') as f:
                        pickle.dump(train_loss_list, f)
        
        if(epoch_loss < loss_min):
            loss_min = epoch_loss
            torch.save(net, path + 'MLP_pretrain_for_brain_size592by592_500K_Minimum_loss'+'.pkl')
...
total_epochs = 0 # 用于统计总的迭代次数，保存文件时需要
total_num = 0
epoch = 0
...
while epoch < n_epochs:
    def closure():
        ...
        global loss_last, last_net, last_net2, count, loss_min, num_of_failed, total_num, total_epochs
        
        epoch_loss = 0
        optimiser.zero_grad()
        adjust_learning_rate(optimiser, total_num, start_lr)
        ...
        for batch in range(n_batch):  # 批处理循环。'n_batch'批次数量（即数据被分成几个批次）（循环遍历每个批次）
            ...
            # 采样和过滤
#             sampling_rate = 8  # 设置采样率为8    # 后续代码中 'rec[:,:,::sampling_rate]'--对接收信号进行采样，以减少数据量。
        ...
        if (((epoch_loss - loss_last)/loss_last > 0.3) or (math.isnan(epoch_loss))): 
            ...
            if(epoch_loss < loss_min):
                loss_min = epoch_loss
                torch.save(net, path + 'MLP_for_phantom_size592by592_500K_Minimum_loss'+'.pkl')
    ...
    epoch_loss, rollback_flag = optimiser.step(closure)

    total_num += 1 # 不论是否回滚，都会递增总的次数，用于调整学习率

    # 只有在没有回滚的情况下才递增 epoch
